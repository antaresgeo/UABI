...
interface AcquisitionsFromProps {
    type?: 'view' | 'edit' | 'create' | 'modal';
    disabled?: boolean;
    formik?: any;
    acquisition_in?: AdquisitionsItf;
    innerRef?: any;
    onChange: (values, count) => void;
    count_in?: number;
    close?: boolean;
}
...
const AcquisitionsFrom: FC<AcquisitionsFromProps> = ({
    ...
    const [count, set_count] = useState<number>(formik?.values?.acquisitions?.length || count_in || 0);
    ...
    const [acquisition, set_acquisition] = useState<any>(null);
    ...
    useEffect(() => {
        if (close) {
            ...
            set_acquisition({ ...initial_values, ...acquisition_in });
        }
    }, [close]);
    ...
    const addAcquisition = (new_acquisition) => {
        if (!is_empty(new_acquisition)) {
            ...
            onChange(new_acquisition, count);
            set_count((c) => c + 1);
            clearAcquisition();
        }
    };
                                    ...
                                    <div className="input-group-prepend">
                                        <LocationModal
                                            ...
                                            onSave={(values) => {
                                                ...
                                                handleChange({
                                                    target: { value: values.city_name, name: 'city_name' },
                                                });
