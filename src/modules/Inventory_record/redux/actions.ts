...
//     request_dispatch(types.example_type, service.example_service(filters));
   ...
   request_dispatch(types.get_list_registrations, service.get_all_registrations());
const get_all_registrations = (filters?) =>
   request_dispatch(types.get_all_registrations, service.get_all_registrations(filters));
const create_registration = (data: registration) =>
   request_dispatch(types.create_registration, service.create_registration(data));
const delete_registration = (id) =>
   request_dispatch(types.delete_registration, service.delete_registration(id));
const get_registration_by_id = (id) =>
   request_dispatch(types.get_registration, service.get_registration_by_id(id));
const update_registration = (id, data: registration) =>
   request_dispatch(types.update_registration, service.update_registration(id, data));
